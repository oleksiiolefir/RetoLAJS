	...
	public void hacerDescarga(File archivo) {
		try {
			...
			if (archivo.getName() == "alojTuristicos.xml") {
				url = new URL(
						"http://opendata.euskadi.eus/contenidos/ds_recursos_turisticos/albergues_de_euskadi/opendata/alojamientos.xml");
			} else if (archivo.getName() == "alojRural.xml") {
				url = new URL(
						"http://opendata.euskadi.eus/contenidos/ds_recursos_turisticos/alojamientos_rurales_euskadi/opendata/alojamientos.xml");
		...
		try {
			...
			for (int i = 0; i < listaUsuarios.getLength(); i++) {
				...
				if (nodo.getNodeType() == Node.ELEMENT_NODE) {
					...
					try {
						aloj1.setTelefono(
								element.getElementsByTagName("phone").item(0).getTextContent().replace(" ", ""));
					} catch (Exception ex) {
						aloj1.setTelefono(" ");
					}
					try {
					...
					try {
						aloj1.setCapacidad(
								Integer.parseInt(element.getElementsByTagName("capacity").item(0).getTextContent()));
					} catch (Exception ex) {
						aloj1.setCapacidad(0);
					}
					try {
						aloj1.setLatitud(
								Float.valueOf(element.getElementsByTagName("latwgs84").item(0).getTextContent()));
					} catch (Exception ex) {
						aloj1.setLatitud(0);
					}
					try {
						aloj1.setLongitud(
								Float.valueOf(element.getElementsByTagName("lonwgs84").item(0).getTextContent()));
					} catch (Exception ex) {
						aloj1.setLongitud(0);
					}
					...
					System.out.println(aloj1.getProvincia());
